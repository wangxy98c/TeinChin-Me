<template>
    <div class="app-container">
       <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
          ...
          <el-form-item label="商机归属人" prop="owner">
             <el-input
       ...
       <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
             <el-button
                ...
                icon="Plus"
                @click="handleAdd"
                v-hasPermi="['tienchin:business:create']"
             >新增</el-button>
          </el-col>
          <el-col :span="1.5">
             <el-button
                ...
                :disabled="single"
                @click="handleUpdate"
                v-hasPermi="['tienchin:business:edit']"
             >修改</el-button>
          </el-col>
          <el-col :span="1.5">
             <el-button
                ...
                :disabled="multiple"
                @click="handleDelete"
                v-hasPermi="['tienchin:business:remove']"
             >删除</el-button>
          </el-col>
          <!-- <el-col :span="1.5">
             <el-button
                ...
                icon="Download"
                @click="handleExport"
                v-hasPermi="['tienchin:business:export']"
             >导出</el-button>
          </el-col> -->
      ...
      <!-- 展示表格 -->
       <el-table v-loading="loading" :data="businessList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="线索编号" align="center" prop="businessId" />
          ...
          <el-table-column label="商机归属" align="center" prop="owner" />
          <el-table-column label="商机状态" align="center" prop="status">
            <template #default="scope">
                <dict-tag :options="business_status" :value="scope.row.status"></dict-tag>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="180" align="center" class-name="small-padding fixed-width">
             <template #default="scope">
                <el-button link type="primary" icon="View" @click="handleBusinessView(scope.row)" v-hasPermi="['tienchin:business:view']">查看</el-button>
                <el-button link type="primary" icon="Pointer" @click="handleAssign(scope.row)" v-hasPermi="['tienchin:business:assign']" v-if="scope.row.owner==userStore.name ||userStore.name=='admin'">分配</el-button>
                <el-button link type="primary" icon="TopRight" @click="handleBusinessFollow(scope.row)" v-hasPermi="['tienchin:business:follow']" v-if="scope.row.owner==userStore.name ||userStore.name=='admin'">跟进</el-button>
             </template>
          </el-table-column>
       ...
       <!-- 分配 -->
       <el-dialog title="分配商机" v-model="assignBusinessDialog" width="500px" append-to-body>
          <el-form ref="businessAssignRef" :model="assignForm" :rules="assignFormRules" label-width="120px">
...
import { getClueById } from "../../../api/tienchin/clue";
 ...
 const {business_status,course_apply_to,course_type,sys_user_sex,clue_level} = proxy.useDict("business_status","course_apply_to","course_type","sys_user_sex","clue_level");
...
//获得有哪些渠道
 ...
 function handleBusinessFollow(data){
    proxy.$router.push('/business/detail/index/'+data.businessId+'/follow')
 }
 //处理分配线索
...
function handleAssign(data){
   ...
   proxy.$refs["businessRef"].validate(valid => {
     if (valid) {
       if (form.value.businessId != undefined) {
        console.log("时间格式：",form.value.nextTime);
         //更新操作
 ...
 function handleDelete(row) {
   ...
   proxy.$modal.confirm('是否确认删除商机编号为"' + businessIds + '"的数据项？').then(function() {
