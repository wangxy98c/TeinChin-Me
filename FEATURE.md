...
- [ ] 线索管理页面，跟进和分配按钮可以根据不同的状态进行隐藏（比如已分配就隐藏分配，伪线索就隐藏跟进）
- [ ] 商机管理，view页面。选择课程类型后会查询，但如果是view。则没有查询操作，导致课程是一个代码。以及沟通重点和沟通纪要的必须提交
- [ ] 添加事务@Transational的地方不要加try-catch。
- [ ] 新建合同中前端删除文件后端也删除（删除的好处在于避免被作为免费的图床占用服务器空间）不删的好处在于能记录更多数据信息（就如同数据库逻辑删除一样），暂不考虑删除
- [ ] 添加合同处，其实可以通过手机号查询数据库来自动填入各种信息。但考虑到有直接线下商谈签合同的情况（没有线索跟进等步骤）允许直接填入。（以后想改再说）
- [ ] WORD转PDF以及前端展示PDF：
...
![image-20240214下午103343546](./FEATURE.assets/image-20240214%E4%B8%8B%E5%8D%88103343546.png)
