        ...
        FastAutoGenerator.create("jdbc:mysql://localhost:3306/tienchin?serverTimezone=Asia/Shanghai&useSSL=false", "root", "12345678")
                .globalConfig(builder -> {
                    builder.author("wangxy") // 设置作者
                            .enableSwagger() // 开启 swagger 模式
                            .fileOverride() // 覆盖已生成文件
                            .outputDir("/Users/wangxy/project/tienchin/tienchin-contract/src/main/java"); // 指定输出目录
                })
                .dataSourceConfig(builder -> builder.typeConvertHandler((globalConfig, typeRegistry, metaInfo) -> {
                    int typeCode = metaInfo.getJdbcType().TYPE_CODE;
                    if (typeCode == Types.SMALLINT) {
                ...
                .packageConfig(builder -> {
                    builder.parent("com.wangxy.tienchin") // 设置父包名
                            ...
                            .pathInfo(Collections.singletonMap(OutputFile.xml, "/Users/wangxy/project/tienchin/tienchin-contract/src/main/resources/mapper")); // 设置mapperXml生成路径
                })
                .strategyConfig(builder -> {
